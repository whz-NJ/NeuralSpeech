...
def main(args):
    ...
    def make_binary_dataset(vocab, input_prefix, output_prefix, lang, num_workers):
        ...
        merge_result(
            Binarizer.binarize(
                input_file, vocab, lambda t: ds.add_item(t), offset=0, end=offsets[1], src_with_werdur=args.src_with_werdur, src_with_nbest_werdur=args.src_with_nbest_werdur,
            )
        )
        if num_workers > 1:
            pool.join()
...
def binarize(args, filename, vocab, output_prefix, lang, offset, end, append_eos=True):
    ...
    res = Binarizer.binarize(
        filename, vocab, consumer, append_eos=append_eos, offset=offset, end=end, src_with_werdur=args.src_with_werdur, src_with_nbest_werdur=args.src_with_nbest_werdur,
    )
    ds.finalize(dataset_dest_file(args, output_prefix, lang, "idx"))
    return res
