...
data_logger_name = "data_logger"
business_logger_name = "bs_logger"
...
def init_log():
    ...
    bs_log_handler = TimedRotatingFileHandler_MP(filename=os.path.join(LOG_PATH, BS_LOG_FILE), when=LOG_ROTATE,
                                                 encoding='UTF-8', backupCount=LOG_BACKUP_COUNT)
    error_log_handler = TimedRotatingFileHandler_MP(filename=os.path.join(LOG_PATH, ERROR_LOG_FILE), when=LOG_ROTATE,
                                                    encoding='UTF-8', backupCount=LOG_BACKUP_COUNT)
    console_handler = logging.StreamHandler()

    console_handler.setLevel(ROOT_LOG_LEVEL)
    bs_log_handler.setLevel(DATA_BS_LOG_LEVEL)
    error_log_handler.setLevel(logging.ERROR)

    logging_format = logging.Formatter(LOG_FORMAT)

    console_handler.setFormatter(logging_format)
    bs_log_handler.setFormatter(logging_format)
