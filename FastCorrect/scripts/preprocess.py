...
def normAndTokenize(line, min_sentence_len=2, split_sentences=False):
    ...
    while idx < len(line):
        ...
        #     english_word += ch
        # else:
        #     tokens.extend(en2cn(english_word))
        #     english_word = ""
        if ch == '°':
            if idx < (len(line) - 1):
                next_ch = Q2B(line[idx + 1])
                if next_ch == 'C' or next_ch == 'c':
                    tokens.extend(list("摄氏度"))
                    prev_ch = next_ch
                    idx += 2
                    continue
            tokens.append("度")
            ...
            if '0' <= prev_ch <= '9' or cn_digit_map.__contains__(prev_ch):
                if is_time:
                    tokens.append("点")
                else:
                    tokens.append("比")
            ...
            tokens.extend(list("等于"))
            prev_ch = ch
            idx += 1
            continue
        elif '0' <= ch <= '9' or ch == '-' or ch == '+': #数字
            chars_num = extract_digits2cn(line, prev_ch, is_time, tokens, idx)
            if chars_num > 0:
                idx += chars_num
                prev_ch = line[idx - 1]
                continue
            ...
            prev_ch = ""
            if not split_sentences: #不分句（处理wiki语料或强制纠错规则时）
                idx += 1
                continue
            ...
            if not g2pM_dict.__contains__(ch):
                g2pM_dict[ch] = unify_pinyin(''.join(model(ch, tone=False, char_split=True)))
            if prev_ch == '时' and ch == '间':
                is_time = True
            elif prev_ch == '比' and ch == '分':
                is_time = False
        prev_ch = ch
        idx = idx + 1
    if len(tokens) >= min_sentence_len:  # 分句且当前句子token数满足要求
